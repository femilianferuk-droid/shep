...
# ==================== SBP Payment (ПОЛНОСТЬЮ ПЕРЕДЕЛАНО) ====================
...
async def cb_sbp_payment(callback: types.CallbackQuery, state: FSMContext):
    ...
    text += "2. Нажмите кнопку «Я оплатил» и отправьте скриншот перевода</blockquote>"
...
async def cb_i_paid_sbp(callback: types.CallbackQuery, state: FSMContext):
    ...
    text += "<blockquote>Пожалуйста, отправьте скриншот перевода (фото).</blockquote>"
...
async def process_sbp_screenshot(message: types.Message, state: FSMContext):
    print(f"[SBP] Получен скриншот от пользователя {message.from_user.id}")
    ...
    # Отправляем скриншот ВСЕМ админам
    for admin_id in ADMIN_IDS:
        try:
            # Создаём клавиатуру для админа
            ...
            print(f"[SBP] Скриншот отправлен админу {admin_id}")
...
async def process_sbp_invalid(message: types.Message):
    print(f"[SBP] Получено не фото от пользователя {message.from_user.id}: {message.content_type}")
    await message.answer(
        "❌ Пожалуйста, отправьте именно скриншот (фото) оплаты.",
