...
SBP_BANK = "ЮМАНИ"
RUB_PER_USDT = 90  # Внутренний курс для CryptoBot
...
async def get_pending_sbp_payments():
    pool = await get_db_pool()
    async with pool.acquire() as conn:
        return await conn.fetch('SELECT * FROM sbp_payments WHERE status = $1 ORDER BY id', 'pending')
...
async def cb_shop(callback: types.CallbackQuery):
    ...
    keyboard.append([InlineKeyboardButton(text="Назад", callback_data="main", icon_custom_emoji_id=EMOJI['back'])])
...
async def cb_category(callback: types.CallbackQuery):
    ...
    keyboard.append([InlineKeyboardButton(text="Назад", callback_data="shop", icon_custom_emoji_id=EMOJI['back'])])
...
async def cb_product(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Купить", callback_data=f"buy_{prod_id}", icon_custom_emoji_id=EMOJI['wallet'])],
        [InlineKeyboardButton(text="Назад", callback_data=f"cat_{product['category_id']}", icon_custom_emoji_id=EMOJI['back'])]
    ])
    ...
    if invoice['status'] == 'paid' and payment and payment['status'] == 'pending':
        ...
        for admin_id in ADMIN_IDS:
            try:
                await bot.send_message(
                    admin_id,
                    f"<b><tg-emoji emoji-id='{EMOJI['money']}'>💰</tg-emoji> Новая покупка (CryptoBot)!</b>\n\n"
...
async def cb_sbp_payment(callback: types.CallbackQuery, state: FSMContext):
    prod_id = int(callback.data.split("_")[1])
    product = await get_product(prod_id)
    ...
    text += "2. Нажмите «Я оплатил» и отправьте скриншот</blockquote>"
...
async def cb_i_paid_sbp(callback: types.CallbackQuery, state: FSMContext):
    # Проверяем что пользователь в нужном состоянии
    current_state = await state.get_state()
    if current_state != UserStates.waiting_sbp_screenshot:
        await callback.answer("Сессия истекла, начните заново", show_alert=True)
        return

    text = f"<b><tg-emoji emoji-id='{EMOJI['media']}'>📸</tg-emoji> Отправьте скриншот оплаты</b>\n\n"
    text += "<blockquote>Пожалуйста, отправьте скриншот перевода.</blockquote>"
...
async def process_sbp_screenshot(message: types.Message, state: FSMContext):
    ...
    if not prod_id or not amount:
        await message.answer("Ошибка данных. Попробуйте заново.", reply_markup=back_button("shop"))
        await state.clear()
        return

    file_id = message.photo[-1].file_id
    payment_id = await save_sbp_payment(message.from_user.id, prod_id, amount, file_id)
        ...
        try:
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="Одобрить",
                        callback_data=f"approve_sbp_{payment_id}",
                        icon_custom_emoji_id=EMOJI['check']
                    ),
                    InlineKeyboardButton(
                        text="Отклонить",
                        callback_data=f"reject_sbp_{payment_id}",
                        icon_custom_emoji_id=EMOJI['cross']
                    )
                ]
            ])
            await bot.send_photo(
                admin_id,
                file_id,
                caption=f"<b><tg-emoji emoji-id='{EMOJI['money']}'>💰</tg-emoji> Новая СБП оплата!</b>\n\n"
    ...
    text = f"<b><tg-emoji emoji-id='{EMOJI['clock']}'>⏰</tg-emoji> Скриншот отправлен на проверку!</b>\n\n"
    text += "<blockquote>Администратор проверит оплату в ближайшее время.</blockquote>"

    await message.answer(text, parse_mode="HTML", reply_markup=back_button("shop"))


@router.message(UserStates.waiting_sbp_screenshot)
async def process_sbp_invalid(message: types.Message):
    await message.answer(
        "❌ Пожалуйста, отправьте скриншот (фото) оплаты.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Отмена", callback_data="shop", icon_custom_emoji_id=EMOJI['cross'])]
        ])
    )
...
async def cb_approve_sbp(callback: types.CallbackQuery):
    ...
    await callback.message.edit_caption(
        caption=f"{callback.message.caption}\n\n<b><tg-emoji emoji-id='{EMOJI['check']}'>✅</tg-emoji> ОДОБРЕНО</b>",
        parse_mode="HTML"
    )
...
async def cb_reject_sbp(callback: types.CallbackQuery):
    ...
    await callback.message.edit_caption(
        caption=f"{callback.message.caption}\n\n<b><tg-emoji emoji-id='{EMOJI['cross']}'>❌</tg-emoji> ОТКЛОНЕНО</b>",
        parse_mode="HTML"
    )
...
async def cb_setmedia(callback: types.CallbackQuery, state: FSMContext):
    ...
    text += f"<blockquote>Отправьте фото, видео или GIF:</blockquote>"
...
async def process_media_file(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f"<b><tg-emoji emoji-id='{EMOJI['check']}'>✅</tg-emoji> Медиа установлено!</b>", parse_mode="HTML", reply_markup=admin_back())
...
async def cb_admin_broadcast(callback: types.CallbackQuery, state: FSMContext):
    ...
    text += f"<blockquote>Отправьте текст, фото, видео или GIF:</blockquote>"
...
async def cb_admin_categories(callback: types.CallbackQuery):
    ...
    for cat in categories:
        keyboard.append([
            InlineKeyboardButton(text=cat['name'], callback_data=f"viewcat_{cat['id']}", icon_custom_emoji_id=EMOJI['categories']),
            InlineKeyboardButton(text="", callback_data=f"delcat_{cat['id']}", icon_custom_emoji_id=EMOJI['delete'])
        ])
    keyboard.append([InlineKeyboardButton(text="Добавить категорию", callback_data="addcat", icon_custom_emoji_id=EMOJI['add'])])
    keyboard.append([InlineKeyboardButton(text="Назад", callback_data="admin_panel", icon_custom_emoji_id=EMOJI['back'])])

    text = f"<b><tg-emoji emoji-id='{EMOJI['categories']}'>📁</tg-emoji> Категории</b>\n\n"
    text += f"<blockquote>Управление категориями:</blockquote>"
...
async def cb_admin_products(callback: types.CallbackQuery):
    ...
    keyboard = []
    for cat in categories:
        keyboard.append([InlineKeyboardButton(text=cat['name'], callback_data=f"admincat_{cat['id']}", icon_custom_emoji_id=EMOJI['categories'])])
    keyboard.append([InlineKeyboardButton(text="Добавить товар", callback_data="addprod", icon_custom_emoji_id=EMOJI['add'])])
    keyboard.append([InlineKeyboardButton(text="Назад", callback_data="admin_panel", icon_custom_emoji_id=EMOJI['back'])])

    text = f"<b><tg-emoji emoji-id='{EMOJI['products']}'>📦</tg-emoji> Товары</b>\n\n"
    text += f"<blockquote>Выберите категорию:</blockquote>"
...
async def cb_admincat(callback: types.CallbackQuery):
    ...
    for prod in products:
        keyboard.append([
            InlineKeyboardButton(text=f"{prod['name']} — {prod['price']:.0f}₽", callback_data=f"viewprod_{prod['id']}", icon_custom_emoji_id=EMOJI['box']),
            InlineKeyboardButton(text="", callback_data=f"delprod_{prod['id']}", icon_custom_emoji_id=EMOJI['delete'])
        ])
    keyboard.append([InlineKeyboardButton(text="Назад", callback_data="admin_products", icon_custom_emoji_id=EMOJI['back'])])
...
async def cb_delprod(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Назад", callback_data="admin_products", icon_custom_emoji_id=EMOJI['back'])]
    ])
...
async def cb_addprod(callback: types.CallbackQuery, state: FSMContext):
    ...
    keyboard = []
    for cat in categories:
        keyboard.append([InlineKeyboardButton(text=cat['name'], callback_data=f"newprodcat_{cat['id']}", icon_custom_emoji_id=EMOJI['categories'])])
    keyboard.append([InlineKeyboardButton(text="Назад", callback_data="admin_products", icon_custom_emoji_id=EMOJI['back'])])
...
async def process_product_name(message: types.Message, state: FSMContext):
    ...
    text = f"<b><tg-emoji emoji-id='{EMOJI['box']}'>📦</tg-emoji> Новый товар</b>\n\n"
    text += f"<blockquote>Введите описание:</blockquote>"
...
async def process_product_price(message: types.Message, state: FSMContext):
    try:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Текстовый", callback_data="prodtype_text", icon_custom_emoji_id=EMOJI['write'])],
            [InlineKeyboardButton(text="Файловый", callback_data="prodtype_file", icon_custom_emoji_id=EMOJI['file'])],
            [InlineKeyboardButton(text="Назад", callback_data="addprod", icon_custom_emoji_id=EMOJI['back'])]
        ])

        text = f"<b><tg-emoji emoji-id='{EMOJI['box']}'>📦</tg-emoji> Новый товар</b>\n\n"
        text += f"<blockquote>Выберите тип товара:</blockquote>"
...
async def cb_prodtype(callback: types.CallbackQuery, state: FSMContext):
    ...
    if prod_type == "text":
        ...
        text = f"<b><tg-emoji emoji-id='{EMOJI['box']}'>📦</tg-emoji> Новый товар</b>\n\n"
        text += f"<blockquote>Введите текстовый контент:</blockquote>"
        ...
        text = f"<b><tg-emoji emoji-id='{EMOJI['box']}'>📦</tg-emoji> Новый товар</b>\n\n"
        text += f"<blockquote>Отправьте файлы (до 10). Нажмите «Готово» когда закончите:</blockquote>"

        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Готово", callback_data="finish_files", icon_custom_emoji_id=EMOJI['check'])],
            [InlineKeyboardButton(text="Назад", callback_data="addprod", icon_custom_emoji_id=EMOJI['back'])]
        ])
...
async def process_product_content(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f"<b><tg-emoji emoji-id='{EMOJI['check']}'>✅</tg-emoji> Товар добавлен!</b>", parse_mode="HTML", reply_markup=admin_back())
...
async def cb_finish_files(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback.message.edit_text(f"<b><tg-emoji emoji-id='{EMOJI['check']}'>✅</tg-emoji> Товар добавлен!</b>", parse_mode="HTML", reply_markup=admin_back())
    await callback.answer()
...
async def cb_admin_settings(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Изменить описание", callback_data="edit_shop_info", icon_custom_emoji_id=EMOJI['edit'])],
        [InlineKeyboardButton(text="Назад", callback_data="admin_panel", icon_custom_emoji_id=EMOJI['back'])]
    ])

    await callback.message.edit_text(f"<b><tg-emoji emoji-id='{EMOJI['settings']}'>⚙️</tg-emoji> Настройки</b>", parse_mode="HTML", reply_markup=keyboard)
    await callback.answer()
...
async def cb_edit_shop_info(callback: types.CallbackQuery, state: FSMContext):
    ...
    text += f"<blockquote>Введите новое описание:</blockquote>"
...
async def process_shop_info(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f"<b><tg-emoji emoji-id='{EMOJI['check']}'>✅</tg-emoji> Описание обновлено!</b>", parse_mode="HTML", reply_markup=admin_back())
