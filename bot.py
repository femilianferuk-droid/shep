...
EMOJI = {
    ...
    "check_user": "5891207662678317861",
    "cross_user": "5893192487324880883",
...
async def init_db():
    ...
    async with pool.acquire() as conn:
        ...
        ''')
        # Product Files (multiple files support)
        await conn.execute('''
...
def main_keyboard():
    return ReplyKeyboardMarkup(
        keyboard=[
            [
                KeyboardButton(text=f"<tg-emoji emoji-id='{EMOJI['shop']}'>🛒</tg-emoji> Купить"),
                KeyboardButton(text=f"<tg-emoji emoji-id='{EMOJI['profile']}'>👤</tg-emoji> Профиль")
            ],
            [
                KeyboardButton(text=f"<tg-emoji emoji-id='{EMOJI['house']}'>🏬</tg-emoji> О шопе"),
                KeyboardButton(text=f"<tg-emoji emoji-id='{EMOJI['support']}'>🛟</tg-emoji> Поддержка")
            ]
        ],
...
def admin_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(
            ...
            icon_custom_emoji_id=EMOJI['products']
        )],
        [InlineKeyboardButton(
            text="Категории",
...
async def cb_buy(callback: types.CallbackQuery):
    ...
    # Convert RUB to USDT for CryptoBot
    usdt_amount = product['price'] / RUB_PER_USDT
...
async def cb_sbp_payment(callback: types.CallbackQuery, state: FSMContext):
    ...
    text += f"<tg-emoji emoji-id='{EMOJI['phone']}'>📱</tg-emoji> <b>Номер телефона:</b> <code>{SBP_PHONE}</code>\n"
    text += f"<tg-emoji emoji-id='{EMOJI['bank']}'>🏦</tg-emoji> <b>Банк:</b> {SBP_BANK}\n"
    text += f"<tg-emoji emoji-id='{EMOJI['money']}'>💵</tg-emoji> <b>Сумма к оплате:</b> {product['price']:.0f}₽\n\n"
    text += "<blockquote>1. Переведите указанную сумму по номеру телефона через СБП\n"
    text += "2. После оплаты нажмите кнопку «Я оплатил» и отправьте скриншот перевода</blockquote>"
...
@router.callback_query(F.data == "i_paid_sbp", UserStates.waiting_sbp_screenshot)
async def cb_i_paid_sbp(callback: types.CallbackQuery):
    ...
    text += "<blockquote>Пожалуйста, отправьте скриншот, подтверждающий перевод.</blockquote>"
        ...
        try:
            ...
            await bot.send_photo(
                ...
                caption=f"<b><tg-emoji emoji-id='{EMOJI['money']}'>💰</tg-emoji> Новая СБП оплата!</b>\n\n"
                        f"👤 Пользователь: @{message.from_user.username or 'Без юзернейма'}\n"
...
async def cb_admin_media(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            ...
            icon_custom_emoji_id=EMOJI['house']
        )],
        [InlineKeyboardButton(
            text="Меню магазина",
            callback_data="setmedia_shop_menu",
            icon_custom_emoji_id=EMOJI['shop']
        )],
        [InlineKeyboardButton(
            text="О шопе",
...
async def cb_prodtype(callback: types.CallbackQuery, state: FSMContext):
    ...
    if prod_type == "text":
        ...
        text = f"<b><tg-emoji emoji-id='{EMOJI['box']}'>📦</tg-emoji> Новый товар</b>\n\n"
        text += f"<blockquote>Введите текстовый контент товара (данные, ключи, инструкции и т.д.):</blockquote>"
        ...
        text = f"<b><tg-emoji emoji-id='{EMOJI['box']}'>📦</tg-emoji> Новый товар</b>\n\n"
        text += f"<blockquote>Отправьте файлы товара (до 10 файлов).\nКогда закончите, нажмите «Готово»:</blockquote>"
...
async def process_product_content(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f"<b><tg-emoji emoji-id='{EMOJI['check']}'>✅</tg-emoji> Товар успешно добавлен!</b>", parse_mode="HTML", reply_markup=admin_back())
