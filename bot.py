...
async def cb_i_paid_sbp(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = f"<b><tg-emoji emoji-id='{EMOJI['photo']}'>📸</tg-emoji> Отправьте скриншот оплаты</b>\n\n"
...
async def process_sbp_screenshot(message: types.Message, state: FSMContext):
    print(f"[DEBUG] ========== ПОЛУЧЕН СКРИНШОТ ==========")
    print(f"[DEBUG] От пользователя: {message.from_user.id}")
    print(f"[DEBUG] Username: @{message.from_user.username}")
    
    data = await state.get_data()
    print(f"[DEBUG] Данные из state: {data}")
    
    prod_id = data.get('sbp_product_id')
    amount = data.get('sbp_amount')

    if not prod_id or not amount:
        print("[DEBUG] ОШИБКА: Нет данных в state")
    ...
    print(f"[DEBUG] Платеж сохранен в БД, ID: {payment_id}")
    
    await state.clear()
    print("[DEBUG] State очищен")

    product = await get_product(prod_id)
    print(f"[DEBUG] Товар: {product['name']}")

    # Отправляем скриншот админам
    for admin_id in ADMIN_IDS:
        print(f"[DEBUG] Отправка админу {admin_id}...")
        try:
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [
                    InlineKeyboardButton(
            ...
            print(f"[DEBUG] ✅ Скриншот успешно отправлен админу {admin_id}")
        except Exception as e:
            print(f"[DEBUG] ❌ Ошибка отправки админу {admin_id}: {e}")

    # Отвечаем пользователю
    text = f"<b><tg-emoji emoji-id='{EMOJI['clock']}'>⏰</tg-emoji> Скриншот отправлен на проверку!</b>\n\n"
    text += "<blockquote>Администратор проверит оплату в ближайшее время.\n"
    text += f"ID платежа: <code>{payment_id}</code></blockquote>"
...
async def process_sbp_invalid(message: types.Message):
    print(f"[DEBUG] Получено НЕ фото в состоянии ожидания скриншота: {message.content_type}")
