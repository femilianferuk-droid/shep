...
EMOJI_USER_X = "5893192487324880883"
...
EMOJI_EYE_HIDE = "6037243349675544634"
...
EMOJI_CRYPTO = "5260752406890711732"
...
EMOJI_FORMAT = "5778479949572738874"
...
EMOJI_BROADCAST = "6039422865189638057"
...
EMOJI_DELETE = "5870657884844462243"
...
async def get_products_by_category(category_id: int):
    pool = await create_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch('SELECT * FROM products WHERE category_id = $1 AND is_active = 1', category_id)
    await pool.close()
    return rows
...
def admin_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(
            ...
            icon_custom_emoji_id=EMOJI_MEDIA
        )],
        [InlineKeyboardButton(
            text="Рассылка",
            callback_data="admin_broadcast",
            icon_custom_emoji_id=EMOJI_MEGAPHONE
        )],
        [InlineKeyboardButton(
            text="Товары",
...
async def cb_buy(callback: types.CallbackQuery):
    ...
    text += f"<blockquote>Нажмите кнопку «Оплатить» и после оплаты нажмите «Проверить оплату»</blockquote>"
...
async def process_broadcast(message: types.Message, state: FSMContext):
    ...
    status_msg = await message.answer(f"<tg-emoji emoji-id='{EMOJI_SEND}'>📤</tg-emoji> Рассылка начата...")
...
async def cb_delprod(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="◁ Назад", callback_data="admin_products", icon_custom_emoji_id=EMOJI_BACK)]
    ])
    await callback.message.edit_text(f"<b><tg-emoji emoji-id='{EMOJI_CHECK}'>✅</tg-emoji> Товар удален</b>", parse_mode="HTML", reply_markup=keyboard)
...
async def process_product_price(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(f"<tg-emoji emoji-id='{EMOJI_CROSS}'>❌</tg-emoji> Введите корректную цену (число)")
