...
async def cb_sbp(callback: types.CallbackQuery, state: FSMContext):
    ...
    payment_id = await save_sbp_payment(callback.from_user.id, prod_id, product["price"])
    await state.update_data(sbp_product_id=prod_id, sbp_payment_id=payment_id, sbp_amount=product["price"])
        ...
        try:
            await bot.send_photo(
                ...
                caption=f'<b><tg-emoji emoji-id="5769126056262898415">🏧</tg-emoji> Новая оплата СБП!</b>\n\n'
                        ...
                        f'<tg-emoji emoji-id="5884479287171485878">📦</tg-emoji> Товар ID: {prod_id}\n'
