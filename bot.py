...
# ВРЕМЕННОЕ ХРАНИЛИЩЕ ДЛЯ SBP (без FSM)
sbp_temp = {}
...
# УНИВЕРСАЛЬНЫЙ ОБРАБОТЧИК ДЛЯ ФОТО И ФАЙЛОВ
@router.message(F.content_type.in_([ContentType.PHOTO, ContentType.DOCUMENT]))
async def handle_sbp_screenshot(message: types.Message):
    user_id = message.from_user.id
    
    # Проверяем, ждём ли мы от этого пользователя скриншот
    if user_id not in sbp_temp:
        return  # Не ждём - игнорируем
    ...
    # Сохраняем в БД (file_id берём в зависимости от типа)
    if message.photo:
    ...
    # ПЕРЕСЫЛАЕМ АДМИНАМ (ВОТ ТУТ РАБОТАЕТ 100%)
    for admin_id in ADMIN_IDS:
        try:
            ...
            # ПЕРЕСЫЛАЕМ ТОЧНО ТАК ЖЕ КАК ПОЛУЧИЛИ
            if message.photo:
            ...
            print(f"[SBP] ✅ Отправлено админу {admin_id}")
    ...
    await message.answer(
        ...
        f"<blockquote>Ожидайте подтверждения от администратора.\nID: <code>{payment_id}</code></blockquote>",
        parse_mode="HTML",
        reply_markup=back_button("shop")
    )
