...
async def cb_sbp_payment(callback: types.CallbackQuery, state: FSMContext):
    ...
    text += "2. Нажмите кнопку «Я оплатил» и отправьте ФАЙЛ скриншота</blockquote>"
...
async def cb_i_paid_sbp(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = f"<b><tg-emoji emoji-id='{EMOJI['file']}'>📎</tg-emoji> Отправьте ФАЙЛ скриншота оплаты</b>\n\n"
    text += "<blockquote>⚠️ Отправьте именно ФАЙЛ (документ), а не фото!</blockquote>"
...
async def process_sbp_document(message: types.Message, state: FSMContext):
    """Обработчик ТОЛЬКО для документов (файлов)"""
    ...
    file_name = message.document.file_name or "screenshot"
    print(f"[SBP] File ID: {file_id}, Name: {file_name}")
    ...
    response_text = (
        f"<b><tg-emoji emoji-id='{EMOJI['clock']}'>⏰</tg-emoji> Файл отправлен на проверку!</b>\n\n"
...
async def process_sbp_invalid(message: types.Message):
    """Обработчик для НЕ-файлов (фото, текст и т.д.)"""
    print(f"[SBP] Получен НЕ файл от пользователя {message.from_user.id}: {message.content_type}")
    
    if message.photo:
        error_text = "❌ Пожалуйста, отправьте ФАЙЛ (документ), а не фото!\n\nКак отправить файл:\n• Нажмите на скрепку 📎\n• Выберите «Файл»\n• Выберите скриншот"
    else:
        error_text = "❌ Пожалуйста, отправьте именно ФАЙЛ (документ) со скриншотом!"
