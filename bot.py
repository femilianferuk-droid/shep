...
async def add_product(category_id, name, description, price, product_type, content=None, file_id=None, file_text=None):
    async with db_pool.acquire() as conn:
        await conn.execute('''
            INSERT INTO products (category_id, name, description, price, product_type, content, file_id, file_text, created_at)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
        ''', category_id, name, description, price, product_type, content, file_id, file_text, datetime.now().isoformat())
...
async def cb_buy_balance(callback: types.CallbackQuery):
    ...
    if product['product_type'] == 'text':
        ...
        if product['file_text']:
            await bot.send_message(
                callback.from_user.id,
                f'<blockquote>{product["file_text"]}</blockquote>',
                parse_mode="HTML"
            )
        await bot.send_message(
            callback.from_user.id,
...
async def process_product_file(message: types.Message, state: FSMContext):
    ...
    await state.set_state(AdminStates.add_product_file_text)
    
    await message.answer(
        '<b><tg-emoji emoji-id="5884479287171485878">📦</tg-emoji> Файл получен!</b>\n\n<blockquote>Теперь отправьте текстовое описание/инструкцию к файлу (или отправьте "-" чтобы пропустить):</blockquote>',
        parse_mode="HTML"
    )


@router.message(AdminStates.add_product_file_text, F.text)
async def process_product_file_text(message: types.Message, state: FSMContext):
    data = await state.get_data()
    
    file_text = message.text if message.text != "-" else None
