...
EMOJI = {
    ...
    "approve": "5870633910337015697",
    "reject": "5870657884844462243"
}
...
async def cb_sbp_payment(callback: types.CallbackQuery, state: FSMContext):
    ...
    text += "2. Нажмите кнопку «Я оплатил» и отправьте скриншот перевода (можно файлом)</blockquote>"
...
async def cb_i_paid_sbp(callback: types.CallbackQuery, state: FSMContext):
    ...
    if current_state != UserStates.waiting_sbp_screenshot.state:
        await callback.answer("Сессия истекла, начните заново", show_alert=True)
        return

    text = f"<b><tg-emoji emoji-id='{EMOJI['file']}'>📎</tg-emoji> Отправьте скриншот оплаты</b>\n\n"
    text += "<blockquote>Пожалуйста, отправьте скриншот перевода (фото или файл).</blockquote>"
...
@router.message(StateFilter(UserStates.waiting_sbp_screenshot), F.photo)
async def process_sbp_photo(message: types.Message, state: FSMContext):
    """Обработчик для фото (скриншотов)"""
    await process_sbp_file(message, state, message.photo[-1].file_id)


@router.message(StateFilter(UserStates.waiting_sbp_screenshot), F.document)
async def process_sbp_document(message: types.Message, state: FSMContext):
    """Обработчик для документов (файлов)"""
    await process_sbp_file(message, state, message.document.file_id)


async def process_sbp_file(message: types.Message, state: FSMContext, file_id: str):
    """Общая функция обработки скриншота (фото или документ)"""
    print(f"[SBP] Получен файл от пользователя {message.from_user.id}")
    ...
    print(f"[SBP] Платёж сохранён в БД, ID: {payment_id}")
    await state.clear()

    product = await get_product(prod_id)

    # Отправляем ФАЙЛ (как документ) всем админам
    for admin_id in ADMIN_IDS:
        try:
            ...
            # Отправляем как ДОКУМЕНТ (файл), а не фото
            ...
            print(f"[SBP] Файл отправлен админу {admin_id} как документ")
            
        except Exception as e:
            print(f"[SBP] Ошибка отправки админу {admin_id}: {e}")
            # Пробуем отправить как фото если документ не получился
            try:
                ...
                print(f"[SBP] Файл отправлен админу {admin_id} как фото (fallback)")
            except Exception as e2:
                print(f"[SBP] Ошибка отправки как фото: {e2}")

    # Отвечаем пользователю
    response_text = (
        f"<b><tg-emoji emoji-id='{EMOJI['clock']}'>⏰</tg-emoji> Скриншот отправлен на проверку!</b>\n\n"
        f"<blockquote>Администратор проверит оплату в ближайшее время.\n"
        f"ID платежа: <code>{payment_id}</code></blockquote>"
    )
    
    await message.answer(response_text, parse_mode="HTML", reply_markup=back_button("shop"))
    print(f"[SBP] Ответ отправлен пользователю {message.from_user.id}")


@router.message(StateFilter(UserStates.waiting_sbp_screenshot))
async def process_sbp_invalid(message: types.Message):
    print(f"[SBP] Получено не фото/файл от пользователя {message.from_user.id}: {message.content_type}")
    await message.answer(
        "❌ Пожалуйста, отправьте скриншот (фото или файл) оплаты.",
...
async def cb_approve_sbp(callback: types.CallbackQuery):
    ...
    try:
        await bot.send_message(
            ...
            f"Товар выдан в чат выше.",
            parse_mode="HTML"
        )
    except Exception as e:
        print(f"[SBP] Ошибка уведомления пользователя: {e}")
    ...
    await callback.answer("✅ Оплата одобрена, товар выдан!")
