...
async def update_payment_status_by_order_id(order_id: str, status: str):
    async with db_pool.acquire() as conn:
        await conn.execute('UPDATE payments SET status = $1 WHERE invoice_id = $2', status, order_id)
...
async def create_rollypay_payment(amount: float, order_id: str, description: str):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=data) as resp:
                result = await resp.json()
                logging.info(f"RollyPay response: {result}")
                return result
    except Exception as e:
        logging.error(f"RollyPay exception: {e}")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers) as resp:
                result = await resp.json()
                logging.info(f"RollyPay check response: {result}")
                return result
    except Exception as e:
        logging.error(f"RollyPay check error: {e}")
    
    return None


def verify_rollypay_signature(body: str, timestamp: str, signature: str) -> bool:
    """Проверка подписи вебхука RollyPay"""
    expected = hmac.new(
...
async def cb_product(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Оплатить CryptoBot", callback_data=f"buycrypto_{prod_id}", icon_custom_emoji_id="5260752406890711732")],
        [InlineKeyboardButton(text="Оплатить Рубли (RollyPay)", callback_data=f"buyrub_{prod_id}", icon_custom_emoji_id="5904462880941545555")],
...
async def cb_buy_crypto(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Проверить оплату", callback_data=f"checkcrypto_{invoice['invoice_id']}", icon_custom_emoji_id="5345906554510012647")],
...
@router.callback_query(F.data.startswith("buyrub_"))
async def cb_buy_rub(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Оплатить RollyPay", url=result['pay_url'], icon_custom_emoji_id="5904462880941545555")],
        [InlineKeyboardButton(text="Проверить оплату", callback_data=f"checkrub_{order_id}", icon_custom_emoji_id="5345906554510012647")],
        [InlineKeyboardButton(text="◀️ Отмена", callback_data=f"prod_{prod_id}")]
    ])
...
@router.callback_query(F.data.startswith("checkrub_"))
...
# Webhook handler for RollyPay
@router.message(F.text.startswith("/webhook"))
async def webhook_handler(message: types.Message):
    # This is a placeholder - actual webhook should be a separate endpoint
    pass
