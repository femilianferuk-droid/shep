...
async def cb_product(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Купить за баланс", callback_data=f"buyball_{prod_id}")],
        [InlineKeyboardButton(text="Оплатить через СБП", callback_data=f"sbp_{prod_id}")],
        [InlineKeyboardButton(text="Оплатить CryptoBot", callback_data=f"buy_{prod_id}")],
...
async def cb_buy_balance(callback: types.CallbackQuery):
    ...
    if user["balance"] < product["price"]:
        await callback.answer(f"<tg-emoji emoji-id=\"5870657884844462243\">❌</tg-emoji> Недостаточно средств! Ваш баланс: {user['balance']:.0f}₽", show_alert=True)
        return
    ...
    if product['product_type'] == 'text':
        ...
        await bot.send_document(callback.from_user.id, product['file_id'],
                                caption="<tg-emoji emoji-id=\"6039451237743595514\">📎</tg-emoji> Ваш товар", parse_mode="HTML",
...
async def cb_buy(callback: types.CallbackQuery):
    ...
    if not invoice:
        await callback.answer("<tg-emoji emoji-id=\"5870657884844462243\">❌</tg-emoji> Ошибка создания платежа. Проверьте токен CryptoBot!", show_alert=True)
        return
...
async def cb_check_payment(callback: types.CallbackQuery):
    ...
    if not invoice:
        await callback.answer("<tg-emoji emoji-id=\"5870657884844462243\">❌</tg-emoji> Ошибка проверки платежа", show_alert=True)
        return
    ...
    if invoice['status'] == 'paid':
        ...
        if payment and payment['status'] == 'pending':
            ...
            await callback.answer("<tg-emoji emoji-id=\"5870657884844462243\">❌</tg-emoji> Товар уже выдан!", show_alert=True)
    else:
        await callback.answer("<tg-emoji emoji-id=\"5870657884844462243\">❌</tg-emoji> Оплата не найдена. Попробуйте позже.", show_alert=True)
...
async def cb_admin_users(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Изменить баланс пользователя", callback_data="edit_user_balance")],
        [InlineKeyboardButton(text="◀️ Назад", callback_data="admin_panel")]
    ])
...
async def cb_delmedia(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback.answer("<tg-emoji emoji-id=\"5870633910337015697\">✅</tg-emoji> Медиа удалено", show_alert=True)
...
async def process_media_file(message: types.Message, state: FSMContext):
    ...
    if message.photo:
        ...
        await message.answer("<tg-emoji emoji-id=\"5870657884844462243\">❌</tg-emoji> Неподдерживаемый формат", parse_mode="HTML", reply_markup=admin_back())
        return
...
async def cb_admin_categories(callback: types.CallbackQuery):
    ...
    for cat in categories:
        keyboard.append([
            ...
            InlineKeyboardButton(text="Удалить", callback_data=f"delcat_{cat['id']}")
        ])
...
async def cb_delcat(callback: types.CallbackQuery):
    ...
    await callback.answer("<tg-emoji emoji-id=\"5870633910337015697\">✅</tg-emoji> Категория удалена", show_alert=True)
...
async def cb_admincat(callback: types.CallbackQuery):
    ...
    for prod in products:
        keyboard.append([
            ...
            InlineKeyboardButton(text="Удалить", callback_data=f"delprod_{prod['id']}")
        ])
...
async def cb_delprod(callback: types.CallbackQuery):
    ...
    await callback.answer("<tg-emoji emoji-id=\"5870633910337015697\">✅</tg-emoji> Товар удален", show_alert=True)
...
async def process_product_price(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("<tg-emoji emoji-id=\"5870657884844462243\">❌</tg-emoji> Введите корректную цену (число)", parse_mode="HTML")
...
@router.callback_query(
    F.data.in_(["admin_panel", "admin_media", "admin_categories", "admin_products", "addprod", "admin_settings", "admin_users"]))
