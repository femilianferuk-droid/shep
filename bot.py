...
async def get_all_user_purchases(user_id: int):
    async with db_pool.acquire() as conn:
        return await conn.fetch('''
            SELECT p.*, pr.name as product_name FROM purchases p 
            JOIN products pr ON p.product_id = pr.id WHERE p.user_id = $1 ORDER BY p.purchased_at DESC
        ''', user_id)
...
async def show_profile(user_id: int, chat_id: int):
    try:
        ...
        if not user:
            await bot.send_message(chat_id, "<tg-emoji emoji-id=\"5870657884844462243\">❌</tg-emoji> Пользователь не найден", parse_mode="HTML")
            return
        
        purchases = await get_user_purchases(user_id)

        text = f'<b><tg-emoji emoji-id="5870994129244131212">👤</tg-emoji> Мой профиль</b>\n\n'
        text += f'<tg-emoji emoji-id="5870772616305839506">🆔</tg-emoji> <b>ID:</b> <code>{user_id}</code>\n'
        ...
        reg_date = user["registered_at"][:10] if user["registered_at"] else "Н/Д"
        text += f'<tg-emoji emoji-id="5890937706803894250">📅</tg-emoji> <b>Регистрация:</b> {reg_date}\n'
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Мои покупки", callback_data="my_purchases", icon_custom_emoji_id="5884479287171485878")],
            [InlineKeyboardButton(text="Вся статистика", callback_data="full_stats", icon_custom_emoji_id="5870921681735781843")]
        ])

        await send_with_media(chat_id, text, "profile_menu", keyboard)
    except Exception as e:
        logging.error(f"Error in show_profile: {e}")
        await bot.send_message(chat_id, "<tg-emoji emoji-id=\"5870657884844462243\">❌</tg-emoji> Ошибка при загрузке профиля", parse_mode="HTML")
...
async def cb_full_stats(callback: types.CallbackQuery):
    purchases = await get_all_user_purchases(callback.from_user.id)
    user = await get_user(callback.from_user.id)
    
    text = f'<b><tg-emoji emoji-id="5870921681735781843">📊</tg-emoji> Полная статистика</b>\n\n'
    text += f'<tg-emoji emoji-id="5769126056262898415">💰</tg-emoji> <b>Баланс:</b> {user["balance"]:.0f}₽\n'
    text += f'<tg-emoji emoji-id="5884479287171485878">📦</tg-emoji> <b>Всего покупок:</b> {user["total_purchases"]}\n'
    text += f'<tg-emoji emoji-id="5904462880941545555">💵</tg-emoji> <b>Всего потрачено:</b> {user["total_spent"]:.0f}₽\n\n'
    
    if purchases:
        text += f'<b><tg-emoji emoji-id="5884479287171485878">📜</tg-emoji> Все покупки:</b>\n'
        for p in purchases:
            text += f'<tg-emoji emoji-id="5884479287171485878">📦</tg-emoji> {p["product_name"]} — {p["price"]:.0f}₽ ({p["purchased_at"][:10]})\n'
    
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=back_button("profile_back"))
    await callback.answer()


@router.callback_query(F.data == "profile_back")
async def cb_profile_back(callback: types.CallbackQuery):
    await callback.message.delete()
    await show_profile(callback.from_user.id, callback.from_user.id)
    await callback.answer()
...
async def process_sbp_screenshot(message: types.Message, state: FSMContext):
    ...
    await send_with_media(message.from_user.id, text, "payment_success", back_button("shop"))
...
async def cb_buy(callback: types.CallbackQuery):
    ...
    if not invoice:
        text = '<b><tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Ошибка создания платежа!</b>\n\n<blockquote>Проверьте токен CryptoBot или попробуйте позже</blockquote>'
        await send_with_media(callback.from_user.id, text, "payment_failed", back_button(f"prod_{prod_id}"))
        try:
            await callback.message.delete()
        except:
            pass
...
async def cb_check_payment(callback: types.CallbackQuery):
    ...
    if invoice['status'] == 'paid':
        ...
        text = '<b><tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Оплата не найдена!</b>\n\n<blockquote>Попробуйте позже или обратитесь в поддержку</blockquote>'
        await send_with_media(callback.from_user.id, text, "payment_failed", None)
        await callback.answer()
